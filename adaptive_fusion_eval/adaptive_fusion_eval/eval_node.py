...
class EvalNode(Node):
    ...
    def fusion_pose_callback(self, msg: PoseWithCovarianceStamped) -> None:
        ...
        fusion_x = float(msg.pose.pose.position.x)
        fusion_y = float(msg.pose.pose.position.y)
        fusion_yaw = quaternion_to_yaw(
            msg.pose.pose.orientation.x,
            msg.pose.pose.orientation.y,
            msg.pose.pose.orientation.z,
            msg.pose.pose.orientation.w,
        ...
        dx = fusion_x - self.latest_ground_truth.x
        dy = fusion_y - self.latest_ground_truth.y
        position_error = math.hypot(dx, dy)
        yaw_error = abs(normalize_angle(fusion_yaw - self.latest_ground_truth.yaw))
